...
// func GetFriendList(uid int64) ([]models.User, error) {
...
// IsFollower 判断是否关注,uid是否关注toUid
func IsFollower(uid int64, toUid int64) (bool, error) {
	var relation []models.Relation
	err := global.MysqlEngine.Where("follower_id=? and following_id=? and is_del=?", toUid, uid, 0).Find(&relation).Error
	return len(relation) > 0, err
}
